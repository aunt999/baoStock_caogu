...
class StockAnalyzer:
    ...
    def create_widgets(self):
        ...
        self.fig = Figure(figsize=(12, 8), facecolor=self.bg_dark)
        ...
        # Right: Info Panel
        right_frame = tk.Frame(main_frame, bg=self.bg_medium, width=380)
        ...
        self.stock_name_label = tk.Label(self.info_frame, text="-- \u8bf7\u8f93\u5165\u80a1\u7968\u4ee3\u7801 --", bg=self.bg_medium, fg=self.accent2, font=("Microsoft YaHei", 14, "bold"))
        self.stock_name_label.pack(anchor=tk.W, pady=(10, 0))
        
        self.stock_price_label = tk.Label(self.info_frame, text="", bg=self.bg_medium, fg=self.text_white, font=("Consolas", 22, "bold"))
        self.stock_price_label.pack(anchor=tk.W)
        
        self.stock_change_label = tk.Label(self.info_frame, text="", bg=self.bg_medium, fg=self.text_gray, font=("Microsoft YaHei", 11))
        ...
        # Current indicators
        tk.Label(right_frame, text=">> \u5f53\u524d\u6307\u6807", bg=self.bg_medium, fg=self.accent2, font=("Microsoft YaHei", 11, "bold")).pack(anchor=tk.W, padx=10, pady=(5, 3))
        
        self.indicators_frame = tk.Frame(right_frame, bg=self.bg_medium)
        self.indicators_frame.pack(fill=tk.X, padx=10)
        
        self.ind_labels = {}
        for name in ["MA5", "MA10", "MA20", "MA60", "RSI", "MACD", "K", "D", "J"]:
            f = tk.Frame(self.indicators_frame, bg=self.bg_medium)
            f.pack(fill=tk.X, pady=1)
            tk.Label(f, text=name + ":", bg=self.bg_medium, fg=self.text_gray, font=("Consolas", 10), width=6, anchor=tk.W).pack(side=tk.LEFT)
            lbl = tk.Label(f, text="--", bg=self.bg_medium, fg=self.text_white, font=("Consolas", 10), anchor=tk.W)
        ...
        # Signal summary
        tk.Label(right_frame, text=">> \u4fe1\u53f7\u6982\u89c8", bg=self.bg_medium, fg=self.accent2, font=("Microsoft YaHei", 11, "bold")).pack(anchor=tk.W, padx=10, pady=(5, 3))
        
        self.signal_summary_frame = tk.Frame(right_frame, bg=self.bg_medium)
        self.signal_summary_frame.pack(fill=tk.X, padx=10)
        
        self.current_signal_label = tk.Label(self.signal_summary_frame, text="--", bg=self.bg_medium, fg=self.text_gray, font=("Microsoft YaHei", 20, "bold"))
        self.current_signal_label.pack(anchor=tk.W, pady=5)
        
        self.signal_detail_label = tk.Label(self.signal_summary_frame, text="", bg=self.bg_medium, fg=self.text_gray, font=("Microsoft YaHei", 10))
        self.signal_detail_label.pack(anchor=tk.W)
        
        tk.Frame(right_frame, bg=self.bg_light, height=1).pack(fill=tk.X, padx=10, pady=5)
        
        # Backtest result
        tk.Label(right_frame, text=">> \u56de\u6d4b\u7ed3\u679c", bg=self.bg_medium, fg=self.gold_color, font=("Microsoft YaHei", 11, "bold")).pack(anchor=tk.W, padx=10, pady=(5, 3))
        
        self.backtest_frame = tk.Frame(right_frame, bg=self.bg_medium)
        self.backtest_frame.pack(fill=tk.X, padx=10)
        
        self.backtest_labels = {}
        for name in ["\u521d\u59cb\u8d44\u91d1", "\u6700\u7ec8\u8d44\u4ea7", "\u603b\u6536\u76ca", "\u6536\u76ca\u7387", "\u4ea4\u6613\u6b21\u6570", "\u80dc\u7387"]:
            f = tk.Frame(self.backtest_frame, bg=self.bg_medium)
            f.pack(fill=tk.X, pady=1)
            tk.Label(f, text=name + ":", bg=self.bg_medium, fg=self.text_gray, font=("Microsoft YaHei", 10), width=8, anchor=tk.W).pack(side=tk.LEFT)
            lbl = tk.Label(f, text="--", bg=self.bg_medium, fg=self.text_white, font=("Consolas", 10, "bold"), anchor=tk.W)
            lbl.pack(side=tk.LEFT)
        ...
        # Signal list
        tk.Label(right_frame, text=">> \u4fe1\u53f7\u5217\u8868", bg=self.bg_medium, fg=self.accent2, font=("Microsoft YaHei", 11, "bold")).pack(anchor=tk.W, padx=10, pady=(5, 3))
        
        tree_frame = tk.Frame(right_frame, bg=self.bg_medium)
        tree_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))
        
        self.tree = ttk.Treeview(tree_frame, columns=("date", "type", "price", "reason"), show="headings", height=8)
        ...
        self.tree.column("price", width=55)
    ...
    def _draw_chart(self):
        ...
        ax_kline.grid(True, alpha=0.1, color="gray")
        ...
        ax_vol.grid(True, alpha=0.1, color="gray")
        ...
        ax_rsi.grid(True, alpha=0.1, color="gray")
        ...
        ax_macd.grid(True, alpha=0.1, color="gray")
        ...
        ax_kdj.grid(True, alpha=0.1, color="gray")
    ...
    def _setup_sync_crosshair(self):
        """Setup synchronized crosshair across all subplots"""
        self.cursor_vlines = []
        self.cursor_info_box = None
        
        # Create vertical lines for each subplot
        for ax in self.axes_list:
            vline = ax.axvline(x=0, color="yellow", linewidth=0.8, alpha=0.6, visible=False, linestyle="-")
            self.cursor_vlines.append(vline)
        
        # Info box on K-line chart
        self.cursor_info_box = self.axes_list[0].text(0.02, 0.98, "", transform=self.axes_list[0].transAxes,
    ...
    def _on_mouse_move_sync(self, event):
        """Handle mouse movement - sync crosshair across all subplots"""
        ...
        # Update info box
        row = self.df.iloc[x]
        info_text = f"{row['date']}\n"
        info_text += f"O:{row['open']:.2f} H:{row['high']:.2f}\n"
        info_text += f"L:{row['low']:.2f} C:{row['close']:.2f}\n"
        info_text += f"VOL:{row['volume']/10000:.0f}\u4e07"
        
        if "MA5" in row and pd.notna(row["MA5"]):
            info_text += f"\nMA5:{row['MA5']:.2f}"
        if "RSI" in row and pd.notna(row["RSI"]):
            info_text += f" RSI:{row['RSI']:.1f}"
        ...
        self.cursor_info_box.set_text(info_text)
